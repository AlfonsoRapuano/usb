...
class ImportINVWithPayment(models.TransientModel):
    ...
    payment_option = fields.Selection([('partial','Invoice Keep Open'),('write_off','Write Off')],default='partial',string="Payment Type")
    write_off_account_id = fields.Many2one('account.account',string='Post Difference In')
    account_option = fields.Selection([('default','Auto'),('sheet','As per sheet')],default='default',string="Account")
                                        ...
                                        if self.payment_option == 'partial':
                                            ...
                                            if created_inv and len(journals) > 0 and len(payment_amount) > 0 and len(journals) == len(payment_amount):
                                                ...
                                                for i in range(0,len(journals)):
                                                    ...
                                                    journal_id = self.env['account.journal'].sudo().search([('name','=',journals[i]),('company_id','=',self.env.company.id)],limit=1)
                                                    if journal_id:
                                                        payment_dic.update({
                                                            'journal_id':journal_id.id,
                                                            })
                                                        list_pay_dic.append(payment_dic)
                                                dic_acount_payment.update({created_inv.id:list_pay_dic})
                                                journals = []
                                                payment_amount = []
                                                payment_date = False
                                ...
                                if self.payment_option == 'partial':
                                    if row[10] not in (None,""):
                                        ...
                                        for journal in journals_split:
                                            account_journal_id = self.env['account.journal'].sudo().search([('name','=',journal),('company_id','=',self.env.company.id)],limit=1)
                                            if account_journal_id:
                                                journals.append(str(account_journal_id.name))
                                            else:
                                                skipped_line_no[str(
                                    ...
                                    if row[10] not in (None,""):
                                        ...
                                        if ',' in row_10:
                                            ...
                                            account_journal_id = self.env['account.journal'].sudo().search([('name','=',row_10),('company_id','=',self.env.company.id)],limit=1)
                                            if account_journal_id:
                                ...
                                if created_inv:
                                    ...
                                    if search_product:
                                        ...
                                        if self.account_option == 'default':
                                            ...
                                            account_id = self.env['account.account'].sudo().search([('code','=',row[3]),('company_id','=',self.env.company.id)],limit=1)
                                            if account_id:
                                                account = account_id
                    ...
                    if created_inv_list_for_validate and self.is_validate:
                        ...
                        if invoices:
                            for invoice in invoices:
                                invoice.action_post()
                                # make payment
                                list_payment_dic = dic_acount_payment.get(invoice.id,False)
                                if list_payment_dic:
                                    for payment_dic in list_payment_dic:
                                        register_id = self.env['account.payment.register'].with_context(active_model='account.move',active_ids=invoice.ids).create(payment_dic)
                            ...
                            if sheet.cell(row, 0).value not in (None, "") and sheet.cell(row, 4).value not in (None, ""):
                                ...
                                if self.payment_option == 'partial':
                                    if sheet.cell(row, 10).value not in (None,""):
                                        row_10 = str(sheet.cell(row, 10).value)
                                        journals_split = row_10.split(',')
                                        for journal in journals_split:
                                            account_journal_id = self.env['account.journal'].sudo().search([('name','=',journal),('company_id','=',self.env.company.id)],limit=1)
                                            if account_journal_id:
                                    ...
                                    if sheet.cell(row, 11).value not in (None,""):
                                        row_11 = str(sheet.cell(row, 11).value)
                                        payment_split = row_11.split(',')
                                        for payment in payment_split:
                                            payment_amount.append(payment)
                                elif self.payment_option == 'write_off':
                                    if sheet.cell(row, 10).value not in (None,""):
                                        ...
                                        if ',' in row_10:
                                            ...
                                            journal = self.env['account.journal'].sudo().search([('name','=',row_10),('company_id','=',self.env.company.id)],limit=1)
                                            if journal:
                                                journal_id = journal
                                            else:
                                                skipped_line_no[str(
                                    ...
                                    if search_product:
                                        ...
                                        if self.account_option == 'default':
                                            ...
                                            if isinstance(sheet.cell(row, 3).value, float):
                                                code = int(sheet.cell(row, 3).value)
                                                code_str = str(code)
                                                account_id = self.env['account.account'].sudo().search([('code','=',code_str),('company_id','=',self.env.company.id)],limit=1)
                                                if account_id:
                                                    account = account_id
                                            else:
                                                account_id = self.env['account.account'].sudo().search([('code','=',sheet.cell(row, 3).value),('company_id','=',self.env.company.id)],limit=1)
                                                if account_id:
                                                    account = account_id
                                        if not account:
                                            skipped_line_no[str(
